...
export default async function RemoteNavbar() {
  ...
  const brand = component.render?.brand as string | undefined ?? "Microtronic Thailand";
  const badge = component.render?.badge as string | undefined ?? "Donation & Payment";
  const actionLabel = component.render?.actionLabel as string | undefined ?? "Micro Donate & Payment";
  const actionHref = component.render?.actionHref as string | undefined ?? "https://microtronic-thailand.github.io/micro-payment/";
  const subtitle = component.render?.subtitle as string | undefined ?? "Support creators and communities with secure micro donations.";
